...
ASX_API asx_socket_addr asx_socket_addr_ipv4(
...
ASX_API int asx_socket_addr_eq(
    const asx_socket_addr *a, const asx_socket_addr *b);
...
ASX_API ASX_MUST_USE asx_status asx_tcp_listener_bind(
    asx_tcp_listener *out, const asx_socket_addr *addr);

/* Poll for incoming connections.
 * Walking skeleton: always returns ASX_E_PENDING (no real accept). */
ASX_API ASX_MUST_USE asx_status asx_tcp_listener_poll_accept(
    asx_tcp_listener listener, asx_tcp_stream *out,
...
ASX_API asx_status asx_tcp_listener_local_addr(
    asx_tcp_listener listener, asx_socket_addr *out);
...
ASX_API ASX_MUST_USE asx_status asx_tcp_connect(
    asx_tcp_stream *out, const asx_socket_addr *addr);

/* Poll-read from a TCP stream.
 * Walking skeleton: always returns ASX_E_PENDING. */
ASX_API ASX_MUST_USE asx_status asx_tcp_stream_poll_read(
    asx_tcp_stream stream, asx_buf_mut *dst, uint32_t *bytes_read);

/* Poll-write to a TCP stream.
 * Walking skeleton: always returns ASX_E_PENDING. */
ASX_API ASX_MUST_USE asx_status asx_tcp_stream_poll_write(
    asx_tcp_stream stream, const asx_buf *src, uint32_t *bytes_written);

/* Close a TCP stream. */
ASX_API asx_status asx_tcp_stream_close(asx_tcp_stream stream);

/* Get the remote address of a stream. */
ASX_API asx_status asx_tcp_stream_peer_addr(
    asx_tcp_stream stream, asx_socket_addr *out);
