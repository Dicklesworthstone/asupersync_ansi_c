...
ASX_API ASX_MUST_USE asx_status asx_snapshot_take(
    const asx_lab *lab,
    asx_snapshot_id *out);

/* Restore a lab to a previously saved snapshot. */
ASX_API asx_status asx_snapshot_restore(
    asx_lab *lab,
    asx_snapshot_id id);
...
ASX_API asx_oracle_result asx_oracle_quiescence(
    const asx_lab *lab, void *ctx);

/* Leak oracle: checks that all regions and obligations are resolved. */
ASX_API asx_oracle_result asx_oracle_leak(
    const asx_lab *lab, void *ctx);

/* Replay oracle: checks that two runs produce identical results. */
ASX_API asx_oracle_result asx_oracle_replay_match(
...
ASX_API asx_status asx_minimize_init(
...
ASX_API const asx_lab_scenario *asx_minimize_result(
