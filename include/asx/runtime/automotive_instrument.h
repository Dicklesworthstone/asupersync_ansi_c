...
ASX_API void asx_auto_deadline_record(asx_auto_deadline_tracker *dt, uint64_t deadline_ns,
                                      uint64_t actual_ns);

/* Get deadline miss rate as percentage * 100 (e.g., 250 = 2.5%).
 * Returns 0 if no deadlines recorded. */
ASX_API uint32_t asx_auto_deadline_miss_rate(const asx_auto_deadline_tracker *dt);
...
ASX_API void asx_auto_audit_record(asx_auto_audit_ring *ring, asx_audit_kind kind,
                                   uint64_t timestamp_ns, uint64_t entity_id, int64_t detail);

/* Get an audit entry by ring index (0 = oldest available).
 * Returns NULL if index out of range. */
ASX_API const asx_audit_entry *asx_auto_audit_get(const asx_auto_audit_ring *ring, uint32_t index);
...
ASX_API void asx_auto_compliance_evaluate(const asx_auto_compliance_gate *gate,
...
ASX_API void asx_auto_record_deadline(uint64_t deadline_ns, uint64_t actual_ns, uint64_t entity_id);

/* Convenience: record a checkpoint in the global watchdog
 * and auto-log violations to the audit ring. */
ASX_API void asx_auto_record_checkpoint(uint64_t now_ns, uint64_t entity_id);
