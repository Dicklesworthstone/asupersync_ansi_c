...
int main(void) {
    ...
    if (asx_tcp_listener_poll_accept(listener, &stream, NULL) != ASX_E_PENDING ||
        asx_tcp_stream_poll_write(stream, &payload, &io_n) != ASX_E_PENDING ||
        ...
        fprintf(stderr, "expected deterministic pending IO contract\n");
        asx_runtime_shutdown(&runtime);
        return 1;
    }
