...
static void scenario_poison_during_storm(void) {
    ...
    SCENARIO_CHECK(asx_task_spawn(r_faulted, poll_checkpoint_cooperative, NULL, &tid) == ASX_OK,
                   "spawn_faulted");
...
static void scenario_cancel_under_load(void) {
    ...
    for (i = 0; i < 8; i++) {
        asx_status rc = asx_task_spawn_captured(rid, poll_yield_once, (uint32_t)sizeof(yield_state),
                                                NULL, &tids[i], &states[i]);
        if (rc != ASX_OK) break;
