...
TEST(compile_time_profile_contract) {
    ASSERT_EQ(ASX_BROWSER_PROFILE_MODE_COUNT, 2);
    ASSERT_EQ(ASX_HAS_BROWSER_IO_SUBPROFILE_SPLIT, 1);
    ASSERT_EQ(ASX_HAS_BROWSER_TRACE_SUBPROFILE_SPLIT, 1);
    ASSERT_EQ(ASX_HAS_TLS_NATIVE_ROOTS, 0);
    ASSERT_EQ(ASX_HAS_TLS_WEBPKI_ROOTS, 0);
    ASSERT_EQ(ASX_HAS_TLS_ROOT_VARIANTS, 0);
    ASSERT_EQ(ASX_HAS_DB_SQLITE_BACKEND, 0);
    ASSERT_EQ(ASX_HAS_DB_POSTGRES_BACKEND, 0);
    ASSERT_EQ(ASX_HAS_DB_MYSQL_BACKEND, 0);
    ASSERT_EQ(ASX_HAS_DB_BACKEND_VARIANTS, 0);
...
#else
    ASSERT_EQ(ASX_HAS_BROWSER_IO, 1);
...
TEST(browser_blocked_count) {
    ...
    ASSERT_EQ(
        (int)blocked,
        10); /* fs, process, signal, io_driver, blocking, server, grpc, messaging, tls, database */
}
...
TEST(gate_matches_active_profile) {
    ...
    for (i = 0; i < ASX_SURFACE_COUNT; i++) {
        asx_status expected =
            asx_surface_available_active((asx_host_surface)i) ? ASX_OK : ASX_E_PERMISSION_DENIED;
...
TEST(surface_name_database) { ASSERT_STR_EQ(asx_surface_name(ASX_SURFACE_DATABASE), "database"); }
