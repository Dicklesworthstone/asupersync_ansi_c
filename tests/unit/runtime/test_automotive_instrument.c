...
TEST(deadline_miss_rate_mixed) {
    ...
    for (i = 0; i < 8; i++) { asx_auto_deadline_record(&dt, 1000, 500 + i * 10); }
    for (i = 0; i < 2; i++) { asx_auto_deadline_record(&dt, 1000, 1100 + i * 10); }
...
TEST(audit_record_and_get) {
    ...
    asx_auto_audit_record(&ring, ASX_AUDIT_DEADLINE_MISS, 1000, 42, -200);
...
TEST(audit_ring_wraparound) {
    ...
    for (i = 0; i < ASX_AUTO_AUDIT_RING_SIZE + 10u; i++) {
        asx_auto_audit_record(&ring, ASX_AUDIT_CHECKPOINT_OK, (uint64_t)(i * 100), (uint64_t)i, 0);
    }

    ASSERT_EQ(asx_auto_audit_count(&ring), (uint32_t)ASX_AUTO_AUDIT_RING_SIZE);
    ASSERT_EQ(asx_auto_audit_total(&ring), (uint32_t)(ASX_AUTO_AUDIT_RING_SIZE + 10u));
    ...
    newest = asx_auto_audit_get(&ring, ASX_AUTO_AUDIT_RING_SIZE - 1u);
    ASSERT_TRUE(newest != NULL);
    ASSERT_EQ(newest->entity_id, (uint64_t)(ASX_AUTO_AUDIT_RING_SIZE + 10u - 1u));
}

TEST(audit_kind_str) {
    ASSERT_STR_EQ(asx_audit_kind_str(ASX_AUDIT_REGION_POISONED), "REGION_POISONED");
    ASSERT_STR_EQ(asx_audit_kind_str(ASX_AUDIT_DEADLINE_MISS), "DEADLINE_MISS");
    ASSERT_STR_EQ(asx_audit_kind_str(ASX_AUDIT_WATCHDOG_VIOLATION), "WATCHDOG_VIOLATION");
    ASSERT_STR_EQ(asx_audit_kind_str((asx_audit_kind)99), "UNKNOWN");
}
...
TEST(compliance_pass_all_within_thresholds) {
    ...
    /* 100 hits, 0 misses */
    for (i = 0; i < 100; i++) { asx_auto_deadline_record(&dt, 1000, 500); }
...
TEST(compliance_fail_high_miss_rate) {
    ...
    for (i = 0; i < 5; i++) { asx_auto_deadline_record(&dt, 1000, 500); }
    for (i = 0; i < 5; i++) { asx_auto_deadline_record(&dt, 1000, 2000); }
...
int main(void) {
    test_log_open("unit", "runtime/automotive_instrument", "test_automotive_instrument");
