...
static void test_report_buf_append(void)
{
    ...
    ASSERT(strcmp(asx_report_buf_cstr(&buf), "hello world") == 0,
           "appended string");
...
static void test_report_buf_append_u32(void)
{
    ...
    ASSERT(strcmp(asx_report_buf_cstr(&buf), "0,42,12345") == 0,
           "formatted integers");
}
...
static void test_report_buf_append_hex64(void)
{
    ...
    ASSERT(strcmp(asx_report_buf_cstr(&buf), "0x0000000000000000") == 0,
           "hex zero");
}
...
static void test_report_buf_truncation(void)
{
    ...
    for (i = 0; i < ASX_REPORT_BUF_SIZE / 10; i++) {
        asx_report_buf_append(&buf, "1234567890");
    }
...
static void test_doctor_text_healthy(void)
{
    ...
    ASSERT(strstr(asx_report_buf_cstr(&buf), "[OK]") != NULL,
           "contains OK");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "runtime") != NULL,
           "mentions runtime");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "Overall: OK") != NULL,
           "overall OK");
...
static void test_doctor_text_failing(void)
{
    ...
    ASSERT(strstr(asx_report_buf_cstr(&buf), "[FAIL]") != NULL,
           "contains FAIL");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "Overall: FAIL") != NULL,
           "overall FAIL");
}
...
static void test_doctor_json_healthy(void)
{
    ...
    ASSERT(strstr(asx_report_buf_cstr(&buf), "{\"checks\":[") != NULL,
           "starts with checks array");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "\"overall\":\"OK\"") != NULL,
           "overall OK in JSON");
...
static void test_doctor_render_null_args(void)
{
    ...
    ASSERT(asx_report_doctor_text(NULL, &buf) == ASX_E_INVALID_ARGUMENT,
           "null report");
    ASSERT(asx_report_doctor_text(&doctor, NULL) == ASX_E_INVALID_ARGUMENT,
           "null buf");
    ASSERT(asx_report_doctor_json(NULL, &buf) == ASX_E_INVALID_ARGUMENT,
           "null report json");
    ASSERT(asx_report_doctor_json(&doctor, NULL) == ASX_E_INVALID_ARGUMENT,
           "null buf json");
}
...
static void test_evidence_text(void)
{
    ...
    MUST_OK(asx_evidence_record(&sink, "oracle:leak", ASX_EVIDENCE_PASS,
                                "no leaks detected", 0));
    MUST_OK(asx_evidence_record(&sink, "oracle:quiesce", ASX_EVIDENCE_WARN,
                                "slow convergence", 0));

    asx_report_buf_init(&buf);
    MUST_OK(asx_report_evidence_text(&sink, &buf));

    ASSERT(strstr(asx_report_buf_cstr(&buf), "[PASS]") != NULL,
           "contains PASS");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "[WARN]") != NULL,
           "contains WARN");
...
static void test_evidence_json(void)
{
    ...
    asx_evidence_sink_init(&sink);
    MUST_OK(asx_evidence_record(&sink, "test", ASX_EVIDENCE_FAIL,
                                "failure", 0));

    asx_report_buf_init(&buf);
    MUST_OK(asx_report_evidence_json(&sink, &buf));

    ASSERT(strstr(asx_report_buf_cstr(&buf), "\"entries\":[") != NULL,
           "entries array");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "\"verdict\":\"FAIL\"") != NULL,
           "verdict FAIL");
}
...
static void test_evidence_render_null_args(void)
{
    ...
    ASSERT(asx_report_evidence_text(NULL, &buf) == ASX_E_INVALID_ARGUMENT,
           "null sink");
    ASSERT(asx_report_evidence_text(&sink, NULL) == ASX_E_INVALID_ARGUMENT,
           "null buf");
}
...
static void test_inspection_text(void)
{
    ...
    ASSERT(strstr(asx_report_buf_cstr(&buf), "initialized: yes") != NULL,
           "shows initialized");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "regions:") != NULL,
           "shows regions");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "trace:") != NULL,
           "shows trace");
...
static void test_log_callback(
    asx_log_level level, const char *source,
...
static void test_failure_artifact_healthy(void)
{
    ...
    ASSERT(strstr(asx_report_buf_cstr(&buf), "FAILURE ARTIFACT") != NULL,
           "header present");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "Doctor Report:") != NULL,
           "doctor present");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "Runtime Inspection:") != NULL,
           "inspection present");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "Evidence:") != NULL,
           "evidence present");
    ASSERT(strstr(asx_report_buf_cstr(&buf), "END ARTIFACT") != NULL,
           "footer present");
...
static void test_failure_artifact_uninitialized(void)
{
    ...
    ASSERT(strstr(asx_report_buf_cstr(&buf), "[FAIL]") != NULL,
           "shows failure for uninitialized");
}
...
static void test_failure_artifact_null_args(void)
{
    ...
    ASSERT(asx_report_failure_artifact(NULL, &buf) == ASX_E_INVALID_ARGUMENT,
           "null rt");
    ASSERT(asx_report_failure_artifact(&rt, NULL) == ASX_E_INVALID_ARGUMENT,
           "null buf");
}
...
static void test_e2e_negative_path_resource_exhaustion(void)
{
    ...
    MUST_OK(asx_evidence_record(&sink, "scenario:exhaust",
        ASX_EVIDENCE_FAIL,
...
static void test_e2e_negative_path_cancellation(void)
{
    ...
    MUST_OK(asx_evidence_record(&sink, "scenario:cancel",
        ASX_EVIDENCE_WARN,
