...
TEST(budget_is_past_deadline_null_safe)
{
    ASSERT_FALSE(asx_budget_is_past_deadline(NULL, 100));
}
...
TEST(budget_consume_poll_null_safe)
{
    ASSERT_EQ(asx_budget_consume_poll(NULL), 0u);
}
...
TEST(budget_consume_cost_null_safe)
{
    ASSERT_FALSE(asx_budget_consume_cost(NULL, 1));
}
...
TEST(budget_is_exhausted_null)
{
    ASSERT_TRUE(asx_budget_is_exhausted(NULL));
}

TEST(budget_polls_null_safe)
{
    ASSERT_EQ(asx_budget_polls(NULL), 0u);
}
...
TEST(status_all_error_codes_are_errors)
{
    ...
    static const asx_status error_codes[] = {
        ASX_E_INVALID_ARGUMENT, ASX_E_INVALID_STATE,
        ASX_E_INVALID_TRANSITION,
        ASX_E_REGION_CLOSED, ASX_E_REGION_POISONED,
        ASX_E_POLL_BUDGET_EXHAUSTED,
        ASX_E_OBLIGATION_ALREADY_RESOLVED,
        ASX_E_CANCELLED, ASX_E_WITNESS_PHASE_REGRESSION,
...
TEST(status_all_codes_have_string)
{
    ...
    static const asx_status known[] = {
        ASX_OK, ASX_E_PENDING,
        ASX_E_INVALID_ARGUMENT, ASX_E_INVALID_STATE,
        ASX_E_INVALID_TRANSITION,
        ASX_E_REGION_CLOSED, ASX_E_REGION_POISONED,
        ASX_E_POLL_BUDGET_EXHAUSTED,
