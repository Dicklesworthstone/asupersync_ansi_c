...
| `net` | `partial` | `include/asx/net/net.h`, `src/net/net.c`, `tests/unit/app/test_app.c`, `tests/vignettes/vignette_network.c`, `tests/e2e/e2e_network_surface.c` | Public networking now includes deterministic in-memory loopback accept/read/write plus UDP delivery and resolver ordering, but it is still not OS-backed or near full upstream breadth |
