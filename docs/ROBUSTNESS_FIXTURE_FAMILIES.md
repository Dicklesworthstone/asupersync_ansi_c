...
| `ROBUST-STATIC-ARENA` | static-vs-dynamic allocator backend equivalence, seal, and partial rollback | `DEC-004`, `DS-S01`, `GATE-STATIC-ARENA-PARITY`, `U-STATIC-ARENA-BACKEND` | `schemas/invariant_schema.json` + `P-REQ-005` + `FB-EXH-*` + allocator-seal negative cases | `static-arena-*`, `allocator-seal-*`, `arena-rollback-*` | `rust_vs_c`, `codec_equivalence`, `profile_parity` |
