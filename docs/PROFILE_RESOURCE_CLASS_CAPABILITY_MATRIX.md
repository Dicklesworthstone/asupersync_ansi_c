...
| `io-uring` unsupported on browser builds | `io_uring` remains an explicit compile-time absence in the ANSI C contract | `partial` | `include/asx/asx_config.h` now exposes `ASX_HAS_IO_URING == 0`, and `tests/unit/runtime/test_browser_boundary.c` asserts that absence across browser and non-browser builds; remaining debt is the actual native reactor/io_uring surface rather than leaving the unsupported state implicit |
