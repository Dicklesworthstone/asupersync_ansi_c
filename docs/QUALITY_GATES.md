...
| `GATE-INCIDENT-REPLAY-BUNDLE` | `test_evidence` incident renderer tests; `test_trace` schema compatibility tests; `lint-schema-validation` over `schemas/incident_bundle.schema.json`, `fixtures/incident_bundles/*.json`, `schemas/trace_event.schema.json`, and `fixtures/trace_events/*.json`; `test-e2e-parallel` incident bundle JSONL emission; minimizer targets added by child beads | `bd-v12u.7`, `bd-v12u.8`, `bd-v12u.13` | Emit operator-grade evidence bundles, trace schema, and minimized counterexamples without mutating runtime behavior. |
