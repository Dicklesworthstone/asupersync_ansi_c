...
| `src/runtime/parallel.c` | `walking-skeleton` | Lane scheduler simulation preserves profile/API shape without enabling real parallel execution. `bd-pweu` tracks production graduation. | `bd-pweu.7` plus the `bd-pweu.11` parallel parity gate: true worker dispatch, replay-stable event commits, and single-vs-multi-worker digest parity. |
...
| `src/runtime/seqlock_ebr_spike.c` | `research-spike` | Seqlock + epoch-based reclamation with portable atomic layer. Single-threaded path functional. | `bd-pweu.4`: graduate only after production atomics land and litmus/formal coverage proves concurrent metadata and reclamation safety. |
