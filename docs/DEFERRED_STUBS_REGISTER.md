...
| `src/runtime/parallel.c` | `graduating-internal` | Deterministic worker-lane routing, locality sharding metadata, bounded steal ordering, timed-lane wake promotion, ghost/native reactor readiness pumping, worker drain states, replay-stable commit counters, large-swarm telemetry, RCH-backed worker-count baselines, and formal memory-model proofs are live in the core scheduler. | Native platform adapters must preserve deterministic commit authority before executing lanes concurrently by default. |
