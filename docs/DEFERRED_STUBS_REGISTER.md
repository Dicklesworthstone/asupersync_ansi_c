...
| `src/runtime/parallel.c` | `graduating-internal` | Deterministic worker-lane routing, bounded steal ordering, timed-lane wake promotion, ghost/native reactor readiness pumping, worker drain states, and replay-stable commit counters are live in the core scheduler. `bd-pweu` tracks production graduation. | `bd-pweu.11`: single-vs-multi-worker digest parity gates before any platform adapter may execute worker lanes concurrently. |
