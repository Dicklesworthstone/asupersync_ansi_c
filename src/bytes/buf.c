...
void asx_buf_mut_compact(asx_buf_mut *buf) {
    ...
    if (remaining > 0) { memmove(buf->data, buf->data + buf->rd_pos, remaining); }
    buf->rd_pos = 0;
...
asx_status asx_buf_mut_get_u16_be(asx_buf_mut *buf, uint16_t *out) {
    ...
    *out = (uint16_t)((uint16_t)buf->data[buf->rd_pos] << 8) | (uint16_t)buf->data[buf->rd_pos + 1];
    buf->rd_pos += 2;
    return ASX_OK;
}
...
asx_status asx_buf_mut_get_u32_be(asx_buf_mut *buf, uint32_t *out) {
    ...
    *out = ((uint32_t)buf->data[buf->rd_pos] << 24) | ((uint32_t)buf->data[buf->rd_pos + 1] << 16) |
           ((uint32_t)buf->data[buf->rd_pos + 2] << 8) | (uint32_t)buf->data[buf->rd_pos + 3];
    buf->rd_pos += 4;
    return ASX_OK;
}
...
asx_status asx_buf_mut_get_u64_be(asx_buf_mut *buf, uint64_t *out) {
    ...
    *out = ((uint64_t)buf->data[buf->rd_pos] << 56) | ((uint64_t)buf->data[buf->rd_pos + 1] << 48) |
           ...
           ((uint64_t)buf->data[buf->rd_pos + 6] << 8) | (uint64_t)buf->data[buf->rd_pos + 7];
    buf->rd_pos += 8;
    return ASX_OK;
}
...
asx_buf asx_buf_mut_freeze(const asx_buf_mut *buf) { return asx_buf_mut_readable(buf); }
