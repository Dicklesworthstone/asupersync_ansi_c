...
void asx_hft_histogram_record(asx_hft_histogram *h, uint64_t ns) {
    ...
    bin = ilog2(ns + 1u);
    if (bin >= ASX_HFT_HISTOGRAM_BINS) { bin = ASX_HFT_HISTOGRAM_BINS - 1u; }
    h->bins[bin]++;
}

uint64_t asx_hft_histogram_percentile(const asx_hft_histogram *h, uint32_t pct) {
    uint32_t target;
    uint32_t cumulative;
    uint32_t i;
    ...
    for (i = 0; i < ASX_HFT_HISTOGRAM_BINS; i++) {
        cumulative += h->bins[i];
        if (cumulative >= target) { return bin_lower(i); }
    }
...
void asx_hft_histogram_reset(asx_hft_histogram *h) { asx_hft_histogram_init(h); }

/* -------------------------------------------------------------------
 * Jitter tracker
 * ------------------------------------------------------------------- */

void asx_hft_jitter_init(asx_hft_jitter_tracker *jt, uint32_t recompute_interval) {
    if (!jt) return;
...
void asx_hft_jitter_recompute(asx_hft_jitter_tracker *jt) {
    ...
    if (jt->hist.overflow > 0) {
        uint64_t diff = ASX_HFT_HISTOGRAM_MAX_NS > mean ? ASX_HFT_HISTOGRAM_MAX_NS - mean
...
void asx_hft_jitter_record(asx_hft_jitter_tracker *jt, uint64_t ns) {
    ...
    if (jt->recompute_every == 0 || jt->samples_since >= jt->recompute_every) {
...
void asx_overload_evaluate(const asx_overload_policy *pol, uint32_t used, uint32_t capacity,
    ...
    switch (pol->mode) {
    case ASX_OVERLOAD_REJECT: decision->admit_status = ASX_E_ADMISSION_CLOSED; break;
    ...
    case ASX_OVERLOAD_SHED_OLDEST:
        ...
        if (decision->shed_count > used) { decision->shed_count = used; }
        decision->admit_status = ASX_OK;
        break;

    case ASX_OVERLOAD_BACKPRESSURE: decision->admit_status = ASX_E_WOULD_BLOCK; break;

    default: decision->admit_status = ASX_E_INVALID_ARGUMENT; break;
    }
}
...
void asx_hft_gate_evaluate(const asx_hft_gate *gate, const asx_hft_histogram *hist,
    ...
    if (hist && gate->p99_9_ns > 0) {
        ...
        if (i == ASX_HFT_HISTOGRAM_BINS) { result->actual_p99_9 = ASX_HFT_HISTOGRAM_MAX_NS; }
    ...
    if (hist && gate->p99_99_ns > 0) {
        ...
        if (i == ASX_HFT_HISTOGRAM_BINS) { result->actual_p99_99 = ASX_HFT_HISTOGRAM_MAX_NS; }
