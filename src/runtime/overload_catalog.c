...
/* ASX_CHECKPOINT_WAIVER_FILE("overload-catalog: all loops bounded by ASX_PROFILE_ID_COUNT (9)") */
 ...
 *
 * BROWSER: Reject at 80%, same as FREESTANDING.
 *   WASM sandbox has constrained memory; earlier rejection
 *   prevents GC pressure and OOM in the host embedder.
 * ------------------------------------------------------------------- */
...
static const asx_overload_catalog_entry g_catalog[ASX_PROFILE_ID_COUNT] = {
    ...
    {ASX_PROFILE_ID_POSIX,
     ...
     {{"fixture-overload-posix", "fixture-overload-baseline", NULL, NULL},
      2,
      "GATE-OVERLOAD-POSIX"}},
    /* WIN32 */
    {ASX_PROFILE_ID_WIN32,
     ...
     {{"fixture-overload-win32", "fixture-overload-baseline", NULL, NULL},
      2,
      "GATE-OVERLOAD-WIN32"}},
    /* FREESTANDING */
    {ASX_PROFILE_ID_FREESTANDING,
     ...
     {{"fixture-overload-freestanding", "fixture-overload-baseline", NULL, NULL},
      2,
      "GATE-OVERLOAD-FREESTANDING"}},
    /* EMBEDDED_ROUTER */
    {ASX_PROFILE_ID_EMBEDDED_ROUTER,
     ...
     ASX_DEGRADE_NONE,
     ASX_FORBID_SILENT_DROP | ASX_FORBID_NONDETERMINISTIC | ASX_FORBID_UNBOUNDED_QUEUE |
         ASX_FORBID_LATENCY_SPIKE,
     "Embedded router: aggressive rejection (75%) prevents buffer bloat",
     {{"fixture-overload-embedded", "fixture-overload-baseline", NULL, NULL},
      2,
      "GATE-OVERLOAD-EMBEDDED"}},
    /* HFT */
    {ASX_PROFILE_ID_HFT,
     ...
     {{"fixture-overload-hft", "fixture-hft-microburst", "fixture-overload-baseline", NULL},
      3,
      "GATE-OVERLOAD-HFT"}},
    /* AUTOMOTIVE */
    {ASX_PROFILE_ID_AUTOMOTIVE,
     ...
     {{"fixture-overload-automotive", "fixture-automotive-watchdog", "fixture-overload-baseline",
       NULL},
      3,
      "GATE-OVERLOAD-AUTOMOTIVE"}},
    /* PARALLEL */
    {ASX_PROFILE_ID_PARALLEL,
     ...
     {{"fixture-overload-parallel", "fixture-overload-baseline", NULL, NULL},
      2,
      "GATE-OVERLOAD-PARALLEL"}},
    /* BROWSER */
    {ASX_PROFILE_ID_BROWSER,
     ASX_OVERLOAD_REJECT,
     80,
     0,
     ASX_DEGRADE_NONE,
     ASX_FORBID_SILENT_DROP | ASX_FORBID_NONDETERMINISTIC | ASX_FORBID_UNBOUNDED_QUEUE,
     "Browser: earlier rejection (80%) protects WASM sandbox memory",
     {{"fixture-overload-browser", "fixture-overload-baseline", NULL, NULL},
      2,
      "GATE-OVERLOAD-BROWSER"}}};

/* -------------------------------------------------------------------
 * Query API
 * ------------------------------------------------------------------- */

uint32_t asx_overload_catalog_version(void) { return ASX_OVERLOAD_CATALOG_VERSION; }

uint32_t asx_overload_catalog_count(void) { return ASX_PROFILE_ID_COUNT; }

asx_status asx_overload_catalog_get(asx_profile_id id, const asx_overload_catalog_entry **out) {
...
asx_status asx_overload_catalog_to_policy(asx_profile_id id, asx_overload_policy *out) {
    if (!out) return ASX_E_INVALID_ARGUMENT;
    if ((int)id < 0 || (int)id >= ASX_PROFILE_ID_COUNT) { return ASX_E_INVALID_ARGUMENT; }
...
int asx_overload_catalog_entry_valid(const asx_overload_catalog_entry *entry) {
    ...
    /* Profile ID in range */
    if ((int)entry->profile < 0 || (int)entry->profile >= ASX_PROFILE_ID_COUNT) { return 0; }
    ...
    for (i = 0; i < ASX_PROFILE_ID_COUNT; i++) {
        ...
        if ((int)entry->profile != (int)i) {
            ...
            if (result->violation_count == 1) {
                snprintf(result->first_violation, sizeof(result->first_violation),
                         "catalog[%u]: profile mismatch (expected %u, got %d)", i, i,
                         (int)entry->profile);
            }
            continue;
        }
        ...
        if (!asx_overload_catalog_entry_valid(entry)) {
            ...
            if (result->violation_count == 1) {
                snprintf(result->first_violation, sizeof(result->first_violation),
                         "catalog[%u] (%s): structural validation failed", i,
                         asx_profile_name((asx_profile_id)i));
            }
        }
    }
}
...
int asx_overload_catalog_decision_consistent(const asx_overload_catalog_entry *entry,
    ...
    switch (entry->mode) {
    case ASX_OVERLOAD_REJECT: return decision->admit_status == ASX_E_ADMISSION_CLOSED;
    ...
    case ASX_OVERLOAD_BACKPRESSURE: return decision->admit_status == ASX_E_WOULD_BLOCK;
