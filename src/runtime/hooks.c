...
asx_containment_policy asx_containment_policy_for_profile(asx_safety_profile profile) {
    switch (profile) {
    case ASX_SAFETY_DEBUG: return ASX_CONTAIN_FAIL_FAST;
    case ASX_SAFETY_HARDENED: return ASX_CONTAIN_POISON_REGION;
    case ASX_SAFETY_RELEASE: return ASX_CONTAIN_ERROR_ONLY;
    }
...
asx_status asx_fault_inject(const asx_fault_injection *fault) {
    ...
    if (g_fault_count >= ASX_FAULT_MAX_ACTIVE) return ASX_E_RESOURCE_EXHAUSTED;
...
uint32_t asx_fault_injection_count(void) { return g_fault_count; }
...
asx_status asx_runtime_hooks_validate(const asx_runtime_hooks *hooks, int deterministic_mode) {
    if (!hooks) return ASX_E_INVALID_ARGUMENT;

    /* Allocator is mandatory */
    if (!hooks->allocator.malloc_fn || !hooks->allocator.free_fn) return ASX_E_INVALID_ARGUMENT;

    if (deterministic_mode) {
        /* Deterministic mode requires a logical clock */
        if (!hooks->clock.logical_now_ns_fn) return ASX_E_DETERMINISM_VIOLATION;
...
static int asx_codec_text_nonempty(const char *text) { return text != NULL && text[0] != '\0'; }
...
static asx_status asx_codec_json_scan_string(const char *cursor, const char **out_next) {
    ...
    if (cursor == NULL || out_next == NULL || *cursor != '"') { return ASX_E_INVALID_ARGUMENT; }
    ...
    while (*scan != '\0') {
        ...
        if (*scan == '\\') {
            scan++;
            if (*scan == '\0') { return ASX_E_INVALID_ARGUMENT; }
            if (*scan == 'u') {
                int i;
                for (i = 0; i < 4; i++) {
                    scan++;
                    if (!asx_codec_is_lower_hex(*scan) && !(*scan >= 'A' && *scan <= 'F')) {
...
static asx_status asx_codec_json_scan_number(const char *cursor, const char **out_next) {
    ...
    if (cursor == NULL || out_next == NULL) { return ASX_E_INVALID_ARGUMENT; }
    ...
    if (!has_digits) { return ASX_E_INVALID_ARGUMENT; }
    ...
    if (*scan == '.') {
        ...
        if (!frac_digits) { return ASX_E_INVALID_ARGUMENT; }
    }
    ...
    if (*scan == 'e' || *scan == 'E') {
        ...
        if (!exp_digits) { return ASX_E_INVALID_ARGUMENT; }
    }
...
static asx_status asx_codec_json_scan_array(const char *cursor, const char **out_next,
    ...
    if (*cursor != '[') { return ASX_E_INVALID_ARGUMENT; }
    if (depth > 64u) { return ASX_E_INVALID_ARGUMENT; }
...
static asx_status asx_codec_json_scan_object(const char *cursor, const char **out_next,
    ...
    for (;;) {
        ...
        if (st != ASX_OK) { return st; }
        scan = asx_codec_json_skip_ws(scan);
        if (*scan != ':') { return ASX_E_INVALID_ARGUMENT; }
        scan = asx_codec_json_skip_ws(scan + 1);
        st = asx_codec_json_scan_value(scan, &scan, depth + 1u);
        if (st != ASX_OK) { return st; }
        scan = asx_codec_json_skip_ws(scan);
...
static asx_status asx_codec_json_scan_value(const char *cursor, const char **out_next,
    ...
    switch (*scan) {
    case '"': return asx_codec_json_scan_string(scan, out_next);
    case '{': return asx_codec_json_scan_object(scan, out_next, depth);
    case '[': return asx_codec_json_scan_array(scan, out_next, depth);
    case 't':
...
static int asx_codec_json_value_matches(const char *json_text, char required_first_char) {
    ...
    if (json_text == NULL) { return 0; }

    cursor = asx_codec_json_skip_ws(json_text);
    if (*cursor != required_first_char) { return 0; }
    if (asx_codec_json_scan_value(cursor, &next, 0u) != ASX_OK) { return 0; }
...
static char *asx_codec_strdup_range(const char *begin, size_t len) {
    ...
    if (len > 0u) { memcpy(copy, begin, len); }
    copy[len] = '\0';
    return copy;
}
...
static asx_status asx_codec_json_decode_string(const char *cursor, const char **out_next,
                                               char **out_text) {
    const char *scan;
    const char *readp;
    char *decoded;
    ...
    if (asx_codec_json_scan_string(cursor, &scan) != ASX_OK) { return ASX_E_INVALID_ARGUMENT; }

    max_len = (size_t)(scan - cursor);
    decoded = (char *)malloc(max_len);
    if (decoded == NULL) { return ASX_E_RESOURCE_EXHAUSTED; }
    ...
    while (readp < scan - 1) {
        if (*readp == '\\') {
            ...
            case 'u':
                ...
                readp += 4;
                break;
            default: free(decoded); return ASX_E_INVALID_ARGUMENT;
            }
            readp++;
            continue;
        }
...
static asx_status asx_codec_json_decode_u64(const char *cursor, const char **out_next,
                                            uint64_t *out_value) {
    const char *scan;
    uint64_t value;

    if (cursor == NULL || out_next == NULL || out_value == NULL) { return ASX_E_INVALID_ARGUMENT; }

    scan = cursor;
    if (*scan < '0' || *scan > '9') { return ASX_E_INVALID_ARGUMENT; }

    value = 0u;
    while (*scan >= '0' && *scan <= '9') {
        uint64_t digit = (uint64_t)(*scan - '0');
        if (value > (UINT64_MAX - digit) / 10u) { return ASX_E_INVALID_ARGUMENT; }
...
static asx_status asx_codec_capture_raw_json(const char *cursor, const char **out_next,
                                             char **out_json) {
    const char *start;
    const char *next;
    char *copy;

    if (cursor == NULL || out_next == NULL || out_json == NULL) { return ASX_E_INVALID_ARGUMENT; }

    start = asx_codec_json_skip_ws(cursor);
    if (asx_codec_json_scan_value(start, &next, 0u) != ASX_OK) { return ASX_E_INVALID_ARGUMENT; }

    copy = asx_codec_strdup_range(start, (size_t)(next - start));
    if (copy == NULL) { return ASX_E_RESOURCE_EXHAUSTED; }
    *out_json = copy;
    *out_next = next;
    return ASX_OK;
}

static asx_status asx_codec_decode_provenance_json(const char *cursor, const char **out_next,
    ...
    if (cursor == NULL || out_next == NULL || prov == NULL) { return ASX_E_INVALID_ARGUMENT; }
    ...
    if (seen != ASX_PROV_ALL) { return ASX_E_INVALID_ARGUMENT; }
    *out_next = scan;
    return ASX_OK;
}
...
const char *asx_codec_kind_str(asx_codec_kind codec) {
    switch (codec) {
    case ASX_CODEC_KIND_JSON: return "json";
    case ASX_CODEC_KIND_BIN: return "bin";
    default: return "unknown";
    }
}

asx_status asx_codec_kind_parse(const char *text, asx_codec_kind *out_codec) {
    if (text == NULL || out_codec == NULL) { return ASX_E_INVALID_ARGUMENT; }
...
asx_status asx_canonical_fixture_validate(const asx_canonical_fixture *fixture) {
    ...
    if (strncmp(digest, "sha256:", 7) != 0) { return ASX_E_INVALID_ARGUMENT; }
    digest += 7;
    for (i = 0; i < 64u; i++) {
        if (!asx_codec_is_lower_hex(digest[i])) { return ASX_E_INVALID_ARGUMENT; }
    }
    if (digest[64] != '\0') { return ASX_E_INVALID_ARGUMENT; }
...
static asx_status asx_codec_buffer_reserve(asx_codec_buffer *buf, size_t additional) {
    ...
    if (required <= buf->cap) { return ASX_OK; }

    next_cap = (buf->cap == 0u) ? 128u : buf->cap;
    while (next_cap < required) {
        if (next_cap > ((size_t)-1) / 2u) { return ASX_E_RESOURCE_EXHAUSTED; }
        next_cap *= 2u;
    }

    grown = (char *)realloc(buf->data, next_cap);
    if (grown == NULL) { return ASX_E_RESOURCE_EXHAUSTED; }

    buf->data = grown;
    buf->cap = next_cap;
    if (buf->len == 0u) { buf->data[0] = '\0'; }
    return ASX_OK;
}

asx_status asx_codec_buffer_append_bytes(asx_codec_buffer *buf, const char *bytes, size_t len) {
    asx_status st;

    if (buf == NULL || bytes == NULL) { return ASX_E_INVALID_ARGUMENT; }
...
asx_status asx_codec_buffer_append_cstr(asx_codec_buffer *buf, const char *text) {
    if (text == NULL) { return ASX_E_INVALID_ARGUMENT; }
...
asx_status asx_codec_buffer_append_u64(asx_codec_buffer *buf, uint64_t value) {
    ...
    if (n < 0) { return ASX_E_INVALID_STATE; }
...
asx_status asx_codec_buffer_append_json_string(asx_codec_buffer *buf, const char *text) {
    ...
    while (*p != '\0') {
        switch (*p) {
        case '"': st = asx_codec_buffer_append_cstr(buf, "\\\""); break;
        case '\\': st = asx_codec_buffer_append_cstr(buf, "\\\\"); break;
        case '\b': st = asx_codec_buffer_append_cstr(buf, "\\b"); break;
        case '\f': st = asx_codec_buffer_append_cstr(buf, "\\f"); break;
        case '\n': st = asx_codec_buffer_append_cstr(buf, "\\n"); break;
        case '\r': st = asx_codec_buffer_append_cstr(buf, "\\r"); break;
        case '\t': st = asx_codec_buffer_append_cstr(buf, "\\t"); break;
        default:
            if (*p < 0x20u) {
                char esc[7];
...
asx_status asx_codec_buffer_append_string_field(asx_codec_buffer *buf, int *is_first,
...
asx_status asx_codec_buffer_append_u64_field(asx_codec_buffer *buf, int *is_first, const char *key,
                                             uint64_t value) {
    asx_status st;
...
static asx_status asx_codec_buffer_append_raw_json_field(asx_codec_buffer *buf, int *is_first,
    ...
    if (raw_json == NULL) { return ASX_E_INVALID_ARGUMENT; }

    start = asx_codec_json_skip_ws(raw_json);
    if (*start != expected_prefix) { return ASX_E_INVALID_ARGUMENT; }
    st = asx_codec_json_scan_value(start, &end, 0u);
    if (st != ASX_OK) { return st; }
    if (*asx_codec_json_skip_ws(end) != '\0') { return ASX_E_INVALID_ARGUMENT; }
...
static asx_status asx_codec_buffer_append_provenance(asx_codec_buffer *buf, int *is_first,
...
asx_status asx_codec_encode_fixture_json(const asx_canonical_fixture *fixture,
    ...
    if (fixture == NULL || out_json == NULL) { return ASX_E_INVALID_ARGUMENT; }

    st = asx_canonical_fixture_validate(fixture);
    if (st != ASX_OK) { return st; }

    out_json->len = 0u;
    if (out_json->data != NULL) { out_json->data[0] = '\0'; }
    ...
    if (st != ASX_OK) { return st; }
    st = asx_codec_buffer_append_raw_json_field(out_json, &is_first, "input", fixture->input_json,
                                                '{');
    if (st != ASX_OK) { return st; }
    st = asx_codec_buffer_append_string_field(out_json, &is_first, "profile", fixture->profile);
    if (st != ASX_OK) { return st; }
...
asx_status asx_codec_decode_fixture_json(const char *json, asx_canonical_fixture *out_fixture) {
    ...
    if (json == NULL || out_fixture == NULL) { return ASX_E_INVALID_ARGUMENT; }
    ...
    for (;;) {
        ...
        if (asx_codec_key_equals(key, "codec")) {
            ...
            if ((seen & ASX_F_CODEC) != 0u) {
                ...
                if (st == ASX_OK) { st = asx_codec_kind_parse(codec_text, &parsed.codec); }
                if (st == ASX_OK) { seen |= ASX_F_CODEC; }
            }
            free(codec_text);
            ...
            if ((seen & ASX_F_EXPECTED_ERROR_CODES) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_EXPECTED_ERROR_CODES; }
            }
            ...
            if ((seen & ASX_F_EXPECTED_EVENTS) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_EXPECTED_EVENTS; }
            }
            ...
            if ((seen & ASX_F_EXPECTED_FINAL_SNAPSHOT) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_EXPECTED_FINAL_SNAPSHOT; }
            }
            ...
            if ((seen & ASX_F_FIXTURE_SCHEMA_VERSION) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_FIXTURE_SCHEMA_VERSION; }
            }
            ...
            if ((seen & ASX_F_PROFILE) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_PROFILE; }
            }
            ...
            if ((seen & ASX_F_PROVENANCE) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_PROVENANCE; }
            }
            ...
            if ((seen & ASX_F_SCENARIO_DSL_VERSION) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_SCENARIO_DSL_VERSION; }
            }
            ...
            if ((seen & ASX_F_SCENARIO_ID) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_SCENARIO_ID; }
            }
            ...
            if ((seen & ASX_F_SEMANTIC_DIGEST) != 0u) {
                ...
                if (st == ASX_OK) { seen |= ASX_F_SEMANTIC_DIGEST; }
            }
        } else {
            st = ASX_E_INVALID_ARGUMENT;
        }
...
static asx_status asx_codec_decode_fixture_json_payload(const void *payload, size_t payload_len,
    ...
    if (payload_len + 1u < payload_len) { return ASX_E_RESOURCE_EXHAUSTED; }

    bytes = (const unsigned char *)payload;
    for (i = 0u; i < payload_len; i++) {
        if (bytes[i] == '\0') { return ASX_E_INVALID_ARGUMENT; }
    }
...
enum { ASX_CODEC_BIN_HEADER_SIZE = 11u, ASX_CODEC_BIN_CHECKSUM_SIZE = 4u };
...
static int asx_codec_slice_contains_nul(asx_codec_slice slice) {
    ...
    if (!asx_codec_slice_nonempty(slice)) { return 1; }
    for (i = 0u; i < slice.len; i++) {
        if (slice.ptr[i] == '\0') { return 1; }
    }
    return 0;
}

static int asx_codec_slice_json_validate(asx_codec_slice slice, char expected_first_char) {
    char *copy;
    int ok;

    if (!asx_codec_slice_nonempty(slice) || asx_codec_slice_contains_nul(slice)) { return 0; }
...
static int asx_codec_slice_contains(asx_codec_slice haystack, const char *needle) {
    ...
    if (!asx_codec_slice_nonempty(haystack) || needle == NULL) { return 0; }
    needle_len = strlen(needle);
    if (needle_len == 0u || haystack.len < needle_len) { return 0; }

    for (i = 0u; i + needle_len <= haystack.len; i++) {
        if (memcmp(haystack.ptr + i, needle, needle_len) == 0) { return 1; }
    }
    return 0;
}
...
static int asx_codec_slice_matches(asx_codec_slice slice, const char *expected) {
    ...
    if (!asx_codec_slice_nonempty(slice) || expected == NULL) { return 0; }
    expected_len = strlen(expected);
    if (slice.len != expected_len) { return 0; }
...
static int asx_codec_slice_is_sha256_digest(asx_codec_slice digest) {
    ...
    if (!asx_codec_slice_nonempty(digest) || digest.len != 71u) { return 0; }
    if (memcmp(digest.ptr, "sha256:", 7u) != 0) { return 0; }
    for (i = 7u; i < digest.len; i++) {
        if (!asx_codec_is_lower_hex(digest.ptr[i])) { return 0; }
    }
    return 1;
}
...
static asx_status asx_codec_bin_decode_varint(const unsigned char **cursor,
                                              const unsigned char *end, uint64_t *out_value) {
    uint64_t value;
    uint32_t i;
    ...
    for (i = 0u; i < 10u; i++) {
        ...
        if (scan >= end) { return ASX_E_INVALID_ARGUMENT; }
        byte = *scan++;
        if (i == 9u && (byte & 0xfeu) != 0u) { return ASX_E_INVALID_ARGUMENT; }
...
static asx_status asx_codec_bin_append_string_field(asx_codec_buffer *buf, uint32_t tag,
                                                    const char *value) {
    asx_status st;
    size_t len;

    if (buf == NULL || value == NULL) { return ASX_E_INVALID_ARGUMENT; }
    len = strlen(value);
...
static asx_status asx_codec_bin_append_varint_field(asx_codec_buffer *buf, uint32_t tag,
                                                    uint64_t value) {
    asx_status st;
...
static asx_status asx_codec_bin_parse_frame(const void *frame_payload, size_t frame_len,
    ...
    if (memcmp(bytes, g_asx_codec_bin_magic, 4u) != 0) { return ASX_E_INVALID_ARGUMENT; }
    ...
    if (expected_len != frame_len) { return ASX_E_INVALID_ARGUMENT; }

    if ((out_frame->flags & ASX_CODEC_BIN_FLAG_CHECKSUM_FOOTER) != 0u) {
        uint32_t expected_checksum;
        uint32_t observed_checksum;
        expected_checksum =
            asx_codec_bin_load_u32_be(bytes + expected_len - ASX_CODEC_BIN_CHECKSUM_SIZE);
        observed_checksum =
            asx_codec_bin_checksum32(bytes, expected_len - ASX_CODEC_BIN_CHECKSUM_SIZE);
        if (expected_checksum != observed_checksum) { return ASX_E_INVALID_ARGUMENT; }
    }
...
static asx_status asx_codec_bin_validate_view(const asx_codec_bin_fixture_view *view) {
    if (view == NULL) { return ASX_E_INVALID_ARGUMENT; }
    ...
    if (!asx_codec_slice_is_sha256_digest(view->semantic_digest)) { return ASX_E_INVALID_ARGUMENT; }
    return ASX_OK;
}

asx_status asx_codec_decode_fixture_bin_view(const void *payload, size_t payload_len,
    ...
    enum { ASX_BIN_SEEN_REQUIRED = (1u << 17) - 1u };

    if (out_view == NULL) { return ASX_E_INVALID_ARGUMENT; }
    ...
    while (cursor < end) {
        ...
        if (tag == 0u || tag > 63u) { return ASX_E_INVALID_ARGUMENT; }
        if (tag <= 17u) { bit = 1u << (tag - 1u); }
    ...
    if (seen != ASX_BIN_SEEN_REQUIRED) { return ASX_E_INVALID_ARGUMENT; }
...
static asx_status asx_codec_assign_dup_slice(char **out_text, asx_codec_slice slice) {
    if (out_text == NULL || !asx_codec_slice_nonempty(slice)) { return ASX_E_INVALID_ARGUMENT; }
    *out_text = asx_codec_strdup_range(slice.ptr, slice.len);
    if (*out_text == NULL) { return ASX_E_RESOURCE_EXHAUSTED; }
    return ASX_OK;
}
...
static asx_status asx_codec_fixture_from_bin_view(const asx_codec_bin_fixture_view *view,
    ...
    if (view == NULL || out_fixture == NULL) { return ASX_E_INVALID_ARGUMENT; }
    ...
    if (st != ASX_OK) goto fail;
    st = asx_codec_assign_dup_slice(&parsed.expected_final_snapshot_json,
                                    view->expected_final_snapshot_json);
    if (st != ASX_OK) goto fail;
    st = asx_codec_assign_dup_slice(&parsed.expected_error_codes_json,
                                    view->expected_error_codes_json);
    if (st != ASX_OK) goto fail;
    st = asx_codec_assign_dup_slice(&parsed.semantic_digest, view->semantic_digest);
    if (st != ASX_OK) goto fail;
    st = asx_codec_assign_dup_slice(&parsed.provenance.rust_baseline_commit,
                                    view->rust_baseline_commit);
    if (st != ASX_OK) goto fail;
    st = asx_codec_assign_dup_slice(&parsed.provenance.rust_toolchain_commit_hash,
                                    view->rust_toolchain_commit_hash);
    if (st != ASX_OK) goto fail;
    st = asx_codec_assign_dup_slice(&parsed.provenance.rust_toolchain_release,
                                    view->rust_toolchain_release);
    if (st != ASX_OK) goto fail;
    st = asx_codec_assign_dup_slice(&parsed.provenance.rust_toolchain_host,
                                    view->rust_toolchain_host);
    if (st != ASX_OK) goto fail;
...
static asx_status asx_codec_encode_fixture_bin(const asx_canonical_fixture *fixture,
    ...
    if (fixture == NULL || out_payload == NULL) { return ASX_E_INVALID_ARGUMENT; }
    ...
    out_payload->len = 0u;
    if (out_payload->data != NULL) { out_payload->data[0] = '\0'; }
    ...
    st = asx_codec_bin_append_string_field(out_payload, ASX_CODEC_BIN_TAG_SCENARIO_ID,
                                           fixture->scenario_id);
    if (st != ASX_OK) return st;
    ...
    if (st != ASX_OK) return st;
    st =
        asx_codec_bin_append_string_field(out_payload, ASX_CODEC_BIN_TAG_PROFILE, fixture->profile);
    if (st != ASX_OK) return st;
    st = asx_codec_bin_append_varint_field(out_payload, ASX_CODEC_BIN_TAG_CODEC,
                                           (uint64_t)fixture->codec);
    if (st != ASX_OK) return st;
    st = asx_codec_bin_append_varint_field(out_payload, ASX_CODEC_BIN_TAG_SEED, fixture->seed);
    if (st != ASX_OK) return st;
    st = asx_codec_bin_append_string_field(out_payload, ASX_CODEC_BIN_TAG_INPUT_JSON,
                                           fixture->input_json);
    if (st != ASX_OK) return st;
    st = asx_codec_bin_append_string_field(out_payload, ASX_CODEC_BIN_TAG_EXPECTED_EVENTS_JSON,
                                           fixture->expected_events_json);
    if (st != ASX_OK) return st;
    st = asx_codec_bin_append_string_field(out_payload,
                                           ASX_CODEC_BIN_TAG_EXPECTED_FINAL_SNAPSHOT_JSON,
                                           fixture->expected_final_snapshot_json);
    if (st != ASX_OK) return st;
    ...
    if (st != ASX_OK) return st;
    st =
        asx_codec_bin_append_string_field(out_payload, ASX_CODEC_BIN_TAG_RUST_TOOLCHAIN_COMMIT_HASH,
    ...
    if (payload_len > 0xffffffffu) { return ASX_E_RESOURCE_EXHAUSTED; }
    ...
    st = asx_codec_buffer_append_bytes(out_payload, (const char *)checksum_bytes,
...
static asx_status asx_codec_decode_fixture_bin(const void *payload, size_t payload_len,
                                               asx_canonical_fixture *out_fixture) {
    asx_codec_bin_fixture_view view;
    asx_status st;

    if (out_fixture == NULL) { return ASX_E_INVALID_ARGUMENT; }
...
static const asx_codec_vtable g_asx_codec_json_vtable = {ASX_CODEC_KIND_JSON,
                                                         asx_codec_encode_fixture_json,
                                                         asx_codec_decode_fixture_json_payload};

static const asx_codec_vtable g_asx_codec_bin_vtable = {ASX_CODEC_KIND_BIN,
                                                        asx_codec_encode_fixture_bin,
                                                        asx_codec_decode_fixture_bin};

const asx_codec_vtable *asx_codec_vtable_for(asx_codec_kind codec) {
    switch (codec) {
    case ASX_CODEC_KIND_JSON: return &g_asx_codec_json_vtable;
    case ASX_CODEC_KIND_BIN: return &g_asx_codec_bin_vtable;
    default: return NULL;
    }
}

asx_status asx_codec_encode_fixture(asx_codec_kind codec, const asx_canonical_fixture *fixture,
                                    asx_codec_buffer *out_payload) {
    const asx_codec_vtable *vt;

    vt = asx_codec_vtable_for(codec);
    if (vt == NULL || vt->encode_fixture == NULL) { return ASX_E_INVALID_ARGUMENT; }
    return vt->encode_fixture(fixture, out_payload);
}

asx_status asx_codec_decode_fixture(asx_codec_kind codec, const void *payload, size_t payload_len,
                                    asx_canonical_fixture *out_fixture) {
    const asx_codec_vtable *vt;

    vt = asx_codec_vtable_for(codec);
    if (vt == NULL || vt->decode_fixture == NULL) { return ASX_E_INVALID_ARGUMENT; }
