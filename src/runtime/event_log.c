...
uint32_t asx_event_emit(asx_event_kind kind,
                         uint64_t entity_id,
                         uint64_t parent_id,
                         asx_status status)
{
...
uint32_t asx_event_log_count(void)
{
...
uint64_t asx_event_hash_chain(void)
{
    return g_hash_chain;
}
...
asx_status asx_event_log_to_json(asx_codec_buffer *out)
{
    ...
    limit = g_event_count;
    if (limit > ASX_EVENT_LOG_CAPACITY) {
        limit = ASX_EVENT_LOG_CAPACITY;
    }
    ...
    for (i = 0; i < limit; i++) {
        ...
        s = asx_codec_buffer_append_string_field(out, &first, "kind",
                                                  asx_event_kind_str(e->kind));
        if (s != ASX_OK) return s;

        s = asx_codec_buffer_append_u64_field(out, &first, "entity_id",
                                               e->entity_id);
        if (s != ASX_OK) return s;

        s = asx_codec_buffer_append_u64_field(out, &first, "parent_id",
                                               e->parent_id);
        if (s != ASX_OK) return s;

        s = asx_codec_buffer_append_u64_field(out, &first, "sequence",
                                               (uint64_t)e->sequence);
        if (s != ASX_OK) return s;

        s = asx_codec_buffer_append_u64_field(out, &first, "status",
                                               (uint64_t)(uint32_t)e->status);
        if (s != ASX_OK) return s;
...
asx_status asx_event_replay_verify(const asx_event_record *expected,
                                    ...
                                    asx_replay_divergence *divergence)
{
    ...
    if (actual_count > ASX_EVENT_LOG_CAPACITY) {
        actual_count = ASX_EVENT_LOG_CAPACITY;
    }
    ...
    for (i = 0; i < compare_limit; i++) {
        ...
        if (exp->kind != act->kind ||
            exp->entity_id != act->entity_id ||
            exp->parent_id != act->parent_id ||
            exp->sequence != act->sequence ||
