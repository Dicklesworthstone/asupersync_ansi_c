...
static const asx_adapter_descriptor g_descriptors[ASX_ADAPTER_COUNT] = {
    {ASX_ADAPTER_HFT, ASX_PROFILE_ID_HFT, "HFT_LATENCY",
     "Tail-latency optimized adapter with shed-oldest overload policy"},
    {ASX_ADAPTER_AUTOMOTIVE, ASX_PROFILE_ID_AUTOMOTIVE, "AUTO_COMPLIANCE",
     "Deadline-aware adapter with backpressure and compliance tracking"},
    {ASX_ADAPTER_ROUTER, ASX_PROFILE_ID_EMBEDDED_ROUTER, "ROUTER_QUEUE",
...
uint32_t asx_adapter_count(void) { return (uint32_t)ASX_ADAPTER_COUNT; }

asx_status asx_adapter_get_descriptor(asx_adapter_id id, asx_adapter_descriptor *out) {
...
asx_profile_id asx_adapter_profile(asx_adapter_id id) {
    if ((unsigned)id >= (unsigned)ASX_ADAPTER_COUNT) return ASX_PROFILE_ID_COUNT;
...
static void evaluate_via_catalog(asx_profile_id profile, uint32_t used, uint32_t capacity,
...
static void populate_auto_annotations(asx_adapter_result *res) {
    ...
    res->has_annotations = 1;
    res->annotations.automotive.miss_rate_pct100 = asx_auto_deadline_miss_rate(dt);
    res->annotations.automotive.audit_count = asx_auto_audit_count(ring);
}

static void populate_router_annotations(asx_adapter_result *res, uint32_t used, uint32_t capacity,
                                        int triggered) {
    res->has_annotations = 1;
    res->annotations.router.queue_depth = used;
    res->annotations.router.headroom = (capacity > used) ? capacity - used : 0;
    if (triggered) {
        if (g_router_reject_streak < UINT32_MAX) { g_router_reject_streak++; }
    } else {
...
asx_status asx_adapter_evaluate(asx_adapter_id id, asx_adapter_mode mode, uint32_t used,
    ...
    if (mode == ASX_ADAPTER_MODE_ACCELERATED) {
        switch (id) {
        case ASX_ADAPTER_HFT: populate_hft_annotations(out); break;
        case ASX_ADAPTER_AUTOMOTIVE: populate_auto_annotations(out); break;
        case ASX_ADAPTER_ROUTER:
            populate_router_annotations(out, used, capacity, out->decision.triggered);
            break;
        case ASX_ADAPTER_COUNT: break;
        }
    }
...
asx_status asx_adapter_evaluate_both(asx_adapter_id id, uint32_t used, uint32_t capacity,
                                     asx_adapter_result *fallback_out,
                                     asx_adapter_result *accel_out) {
    asx_status s;

    if (fallback_out == NULL || accel_out == NULL) return ASX_E_INVALID_ARGUMENT;

    s = asx_adapter_evaluate(id, ASX_ADAPTER_MODE_FALLBACK, used, capacity, fallback_out);
    if (s != ASX_OK) return s;

    s = asx_adapter_evaluate(id, ASX_ADAPTER_MODE_ACCELERATED, used, capacity, accel_out);
    return s;
}
...
int asx_adapter_isomorphism_builtin(asx_adapter_id id, asx_isomorphism_result *out) {
...
int asx_adapter_isomorphism_check(asx_adapter_id id, const asx_adapter_scenario *scenarios,
    ...
    for (i = 0; i < count; i++) {
        ...
        asx_adapter_evaluate(id, ASX_ADAPTER_MODE_FALLBACK, scenarios[i].used,
                             scenarios[i].capacity, &fb);

        /* Restore streak state so accelerated mode starts from same state */
        g_router_reject_streak = saved_streak;

        asx_adapter_evaluate(id, ASX_ADAPTER_MODE_ACCELERATED, scenarios[i].used,
                             scenarios[i].capacity, &ac);
        ...
        if (fb.decision_digest == ac.decision_digest) {
            ...
            if (matches == i) { out->divergence_index = i; }
        }
    }
