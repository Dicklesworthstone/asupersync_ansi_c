...
static const asx_profile_descriptor g_descriptors[ASX_PROFILE_ID_COUNT] = {
    /* CORE */
    {ASX_PROFILE_ID_CORE, "CORE", ASX_WAIT_YIELD, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 0, ASX_CLASS_R2, 1024},
    /* POSIX */
    {ASX_PROFILE_ID_POSIX, "POSIX", ASX_WAIT_YIELD, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 0, ASX_CLASS_R2, 1024},
    /* WIN32 */
    {ASX_PROFILE_ID_WIN32, "WIN32", ASX_WAIT_YIELD, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 0, ASX_CLASS_R2, 1024},
    /* FREESTANDING */
    {ASX_PROFILE_ID_FREESTANDING, "FREESTANDING", ASX_WAIT_YIELD, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 1, ASX_CLASS_R2, 256},
    /* EMBEDDED_ROUTER */
    {ASX_PROFILE_ID_EMBEDDED_ROUTER, "EMBEDDED_ROUTER", ASX_WAIT_BUSY_SPIN, ASX_MAX_REGIONS,
     ASX_MAX_TASKS, ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 1, ASX_CLASS_R2, 256},
    /* HFT */
    {ASX_PROFILE_ID_HFT, "HFT", ASX_WAIT_BUSY_SPIN, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 0, 0, ASX_CLASS_R2, 1024},
    /* AUTOMOTIVE */
    {ASX_PROFILE_ID_AUTOMOTIVE, "AUTOMOTIVE", ASX_WAIT_SLEEP, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 0, ASX_CLASS_R2, 1024},
    /* PARALLEL */
    {ASX_PROFILE_ID_PARALLEL, "PARALLEL", ASX_WAIT_YIELD, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 0, ASX_CLASS_R2, 1024},
    /* BROWSER — sandboxed, no native surfaces, allocator sealable, R1 trace */
    {ASX_PROFILE_ID_BROWSER, "BROWSER", ASX_WAIT_YIELD, ASX_MAX_REGIONS, ASX_MAX_TASKS,
     ASX_MAX_OBLIGATIONS, ASX_MAX_TIMERS, 1, 1, ASX_CLASS_R2, 256}};

static void descriptor_apply_class(asx_profile_descriptor *desc, asx_resource_class cls) {
    uint32_t num = g_class_scale_num[(int)cls];
    uint32_t den = g_class_scale_den[(int)cls];

    /* Scale resource limits, ensuring minimum of 1 */
    desc->max_regions = (desc->max_regions * num / den) > 0 ? (desc->max_regions * num / den) : 1;
    desc->max_tasks = (desc->max_tasks * num / den) > 0 ? (desc->max_tasks * num / den) : 1;
    desc->max_obligations =
        (desc->max_obligations * num / den) > 0 ? (desc->max_obligations * num / den) : 1;
    desc->max_timers = (desc->max_timers * num / den) > 0 ? (desc->max_timers * num / den) : 1;
    desc->trace_capacity =
        (desc->trace_capacity * num / den) > 0 ? (desc->trace_capacity * num / den) : 1;
    desc->resource_class = cls;
}

asx_status asx_profile_get_descriptor(asx_profile_id id, asx_profile_descriptor *desc) {
    if (desc == NULL) return ASX_E_INVALID_ARGUMENT;
    if ((int)id < 0 || (int)id >= ASX_PROFILE_ID_COUNT) { return ASX_E_INVALID_ARGUMENT; }
    *desc = g_descriptors[(int)id];
    return ASX_OK;
}

asx_status asx_profile_get_descriptor_for_class(asx_profile_id id, asx_resource_class cls,
                                                asx_profile_descriptor *desc) {
    if (desc == NULL) return ASX_E_INVALID_ARGUMENT;
    if ((int)id < 0 || (int)id >= ASX_PROFILE_ID_COUNT) { return ASX_E_INVALID_ARGUMENT; }
    if ((int)cls < 0 || (int)cls >= ASX_CLASS_COUNT) { return ASX_E_INVALID_ARGUMENT; }
    *desc = g_descriptors[(int)id];
...
static const char *g_class_names[ASX_CLASS_COUNT] = {"R1", "R2", "R3"};

const char *asx_resource_class_name(asx_resource_class cls) {
    if ((int)cls < 0 || (int)cls >= ASX_CLASS_COUNT) { return "UNKNOWN"; }
...
asx_status asx_trace_config_init(asx_trace_config *cfg, asx_resource_class cls) {
...
static const char *g_property_names[ASX_PPROP_COUNT] = {"wait_policy",    "max_regions",
                                                        "max_tasks",      "max_obligations",
                                                        "max_timers",     "ghost_monitors",
                                                        "trace_capacity", "allocator_sealable",
                                                        "resource_class"};

const char *asx_profile_property_name(asx_profile_property prop) {
    if ((int)prop < 0 || (int)prop >= ASX_PPROP_COUNT) { return "unknown"; }
...
static const char *g_rule_names[ASX_SRULE_COUNT] = {
    "lifecycle_transitions", "cancel_protocol", "obligation_linearity",  "deterministic_ordering",
...
const char *asx_semantic_rule_name(asx_semantic_rule rule) {
    if ((int)rule < 0 || (int)rule >= ASX_SRULE_COUNT) { return "unknown"; }
    return g_rule_names[(int)rule];
}

uint32_t asx_profile_semantic_rule_count(void) { return ASX_SRULE_COUNT; }

/* -------------------------------------------------------------------
 * Digest-based parity checking
 * ------------------------------------------------------------------- */

int asx_profile_digest_compare(uint64_t digest_a, asx_profile_id profile_a, uint64_t digest_b,
                               asx_profile_id profile_b, asx_parity_result *out) {
...
int asx_profile_check_parity(uint64_t expected_digest, asx_parity_result *out) {
    uint64_t current;
