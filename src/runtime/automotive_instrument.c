...
void asx_auto_deadline_record(asx_auto_deadline_tracker *dt, uint64_t deadline_ns,
                              uint64_t actual_ns) {
    int64_t margin;
    ...
    if (margin < dt->worst_margin_ns || dt->total_deadlines == 1) { dt->worst_margin_ns = margin; }
...
void asx_auto_deadline_reset(asx_auto_deadline_tracker *dt) { asx_auto_deadline_init(dt); }
...
static const char *g_audit_kind_names[] = {"REGION_POISONED",    "CANCEL_FORCED",  "DEADLINE_MISS",
                                           "WATCHDOG_VIOLATION", "DEGRADED_ENTER", "DEGRADED_EXIT",
                                           "CHECKPOINT_OK"};
...
void asx_auto_audit_record(asx_auto_audit_ring *ring, asx_audit_kind kind, uint64_t timestamp_ns,
                           uint64_t entity_id, int64_t detail) {
    asx_audit_entry *e;
...
const asx_audit_entry *asx_auto_audit_get(const asx_auto_audit_ring *ring, uint32_t index) {
    uint32_t available;
    uint32_t start;

    if (!ring) return NULL;

    available = ring->count < ASX_AUTO_AUDIT_RING_SIZE ? ring->count : ASX_AUTO_AUDIT_RING_SIZE;
...
uint32_t asx_auto_audit_count(const asx_auto_audit_ring *ring) {
    if (!ring) return 0;
    return ring->count < ASX_AUTO_AUDIT_RING_SIZE ? ring->count : ASX_AUTO_AUDIT_RING_SIZE;
}
...
const char *asx_audit_kind_str(asx_audit_kind kind) {
    if ((unsigned)kind < ASX_AUDIT_KIND_COUNT) { return g_audit_kind_names[(unsigned)kind]; }
    return "UNKNOWN";
}

void asx_auto_audit_reset(asx_auto_audit_ring *ring) { asx_auto_audit_init(ring); }
...
void asx_auto_compliance_evaluate(const asx_auto_compliance_gate *gate,
                                  const asx_auto_deadline_tracker *dt, const asx_auto_watchdog *wd,
