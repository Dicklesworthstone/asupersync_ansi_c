...
asx_status asx_runtime_config_validate(const asx_runtime_config *config)
{
    ...
    if (config->size != (uint32_t)sizeof(asx_runtime_config))
        return ASX_E_INVALID_ARGUMENT;
    if (config->max_cancel_chain_depth == 0u)
        return ASX_E_INVALID_ARGUMENT;
    if (config->finalizer_poll_budget == 0u)
        return ASX_E_INVALID_ARGUMENT;
    return ASX_OK;
}
    ...
    const asx_runtime_hooks *hooks)
{
    ...
    if (rt == NULL || config == NULL || hooks == NULL)
        return ASX_E_INVALID_ARGUMENT;
...
asx_status asx_runtime_get_config(
...
asx_status asx_runtime_get_hooks_from(
    const asx_runtime *rt,
...
{
    if (rt == NULL || !asx_runtime_is_initialized(rt))
        return 0u;
...
uint32_t asx_runtime_region_capacity(void)
{
    return ASX_MAX_REGIONS;
}

uint32_t asx_runtime_task_capacity(void)
{
    return ASX_MAX_TASKS;
}

uint32_t asx_runtime_obligation_capacity(void)
{
    return ASX_MAX_OBLIGATIONS;
}
