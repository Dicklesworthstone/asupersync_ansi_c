...
static int all_mask_dead(const asx_supervisor_slot *s, uint32_t mask)
{
    ...
    for (i = 0; i < s->child_count; i++) {
        if ((mask & (1u << i)) && asx_actor_is_alive(s->children[i])) {
            return 0;
        }
    }
    return 1;
...
asx_status asx_supervisor_start(
    ...
    uint32_t child_count)
{
    ...
    if (child_count > ASX_SUPERVISOR_MAX_CHILDREN)
    ...
    st = asx_task_spawn(region, supervisor_poll,
...
int asx_supervisor_child_alive(
