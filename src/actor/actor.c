...
static asx_status actor_poll(void *user_data, asx_task_id self) {
    ...
    if (mb_pop(s, &env)) {
        ...
        case ASX_ACTOR_MSG_CALL:
            if (s->behavior.handle_call != NULL && env.call_slot < ASX_ACTOR_MAX_PENDING_CALLS) {
                uint64_t reply = 0;
                st = s->behavior.handle_call(s->state, env.payload, &reply, handle);
                if (st == ASX_OK) {
        ...
        case ASX_ACTOR_MSG_STOP: s->stop_requested = 1; break;
        }
    }
...
asx_status asx_actor_call(asx_actor_handle actor, uint64_t request, asx_call_token *out_token) {
    asx_actor_slot *s;
    asx_actor_envelope env;
    uint32_t cs;
    ...
    if (!call_slot_alloc(s, &cs)) return ASX_E_RESOURCE_EXHAUSTED;
