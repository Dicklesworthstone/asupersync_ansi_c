...
int asx_task_handle_is_finished(const asx_task_handle *h)
{
    ...
    if (h == NULL)
        return 0;
    if (asx_task_get_state(h->task_id, &state) != ASX_OK)
        return 0;
...
asx_status asx_task_handle_abort_with_kind(
    asx_task_handle *h,
    ...
    asx_budget budget)
{
    ...
    if (region == ASX_INVALID_ID)
        return ASX_E_INVALID_ARGUMENT;
    if (!asx_cx_has_cap(cx, ASX_CAP_SPAWN))
    ...
    asx_task_handle *out_handle)
{
    ...
    if (scope == NULL || poll_fn == NULL || out_handle == NULL)
        return ASX_E_INVALID_ARGUMENT;
    if (!asx_cx_has_cap(&scope->cx, ASX_CAP_SPAWN))
        return ASX_E_PERMISSION_DENIED;
...
asx_status asx_scope_spawn_captured(
    asx_scope *scope,
    asx_task_poll_fn poll_fn,
    uint32_t state_size,
    ...
    void **out_state)
{
    ...
    st = asx_task_spawn_captured(scope->region_id, poll_fn,
                                 state_size, state_dtor,
                                 &tid, out_state);
    if (st != ASX_OK)
        return st;
