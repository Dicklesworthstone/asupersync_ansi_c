...
asx_status asx_broadcast_create(uint32_t capacity,
                                 ...
                                 asx_broadcast_receiver *out_receiver)
{
    ...
    if (capacity == 0 || capacity > ASX_BROADCAST_MAX_CAPACITY)
        return ASX_E_INVALID_ARGUMENT;
    ...
    if (idx == ASX_MAX_BROADCASTS) {
        if (g_slot_count >= ASX_MAX_BROADCASTS)
                                    ...
                                    asx_broadcast_receiver *out_receiver)
{
    ...
    if (sender == NULL || out_receiver == NULL)
        return ASX_E_INVALID_ARGUMENT;
    ...
    if (s->receiver_count >= ASX_BROADCAST_MAX_RECEIVERS)
        return ASX_E_RESOURCE_EXHAUSTED;
...
asx_status asx_broadcast_try_recv(asx_broadcast_receiver *receiver,
                                   uint64_t *out_value)
{
    asx_broadcast_slot *s;
    ...
    oldest_available = 0;
    if (s->write_seq > s->capacity) {
        oldest_available = s->write_seq - s->capacity;
    }
    ...
    *out_value = s->buffer[ring_idx];
    ...
    asx_trace_emit(ASX_TRACE_CHANNEL_RECV,
                   (uint64_t)receiver->slot, *out_value);
